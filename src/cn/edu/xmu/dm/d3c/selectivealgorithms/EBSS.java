...
import cn.edu.xmu.dm.d3c.diversities.InterraterAgreementK;
...
public class EBSS {
	...
	public static double doEBSS(Instances train, double initCorrectRate,
		...
		if (ClassifierNo.size() == 0) {
			...
			diversity = InterraterAgreementK.CalculateK(temp);
			...
			for (j = 0; j < numOfClassifiers; j++) {
				...
				//
				tempDiversity = InterraterAgreementK.CalculateK(tempList);
				//
				ClassifierNo.add(j);
